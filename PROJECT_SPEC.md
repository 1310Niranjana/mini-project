...
1. **Backbone size**: SpectraNet uses EfficientNet-B6; we use EfficientNet-B0.
2. **Frequency branch design**: our FFT pipeline (grayscale → FFT → fftshift → magnitude → log1p → min-max normalization → 3-channel replication → ImageNet normalization) differs from SpectraNet's frequency branch implementation.
